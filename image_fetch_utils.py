...
#-------------------------------------------------------------------------------
def findRelationship(subject_name, predicate, object_name, data_root):
  for img in range(0, data_root.shape[0]):
    ...
    for i in range(0, anno_root.binary_triples.shape[0]):
      ...
      if (rel == predicate):
        ...
        if sub_name == subject_name and obj_name == object_name:
          print "img %i: %s" % (img, data_root[img].image_url)
...
def getQueries(q_root):
  for q in range(0, q_root.shape[0]):
    ...
    print "Q%d: %s %s %s" % (q, subject_name, pred_name, object_name)
